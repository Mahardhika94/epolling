...
a:hover { 
  ...
  <body class="sidebar-mini fixed">
		...
		<div class="col-md-4"></div>
          <div class="col-md-4">
			...
			<h1 class="text-center"><span class="fa fa-users"></span> Tambah Admin</h1>
             <form action="<?php echo site_url('Insert/add_user/'); ?>" method="post">
			  ...
			  <button type="button" id="btn_klik" class="btn btn-primary"><span class="fa fa-check"></span> Simpan</button>
	...
	function tampil(){
		$.ajax({
			type : 'ajax',
			url	 : '<?php echo base_url(); ?>index.php/Admin/get_user',
			async : false,
		        dataType : 'json',
	...
	$.ajax({
		type : "POST",
		url  : "<?php echo base_url('index.php/Admin/del_user'); ?>",
		dataType : "JSON",
		data : {id:id},
		success : function(data){
		//alert('kok');
	...
	$('#btn_klik').on('click', function(){
		var username = $('#username').val();
		var level = $('#level').val();
		var password =$('#password').val();
		//alert(username+level+password);
		 $.ajax({
			type 	 : "POST",
			url 	 : "<?php echo base_url('index.php/Insert/add_user'); ?>",
			dataType : "JSON",
			data : {username:username , level:level, password:password},
		    success	 : function(data){
			    $('[name="username"]').val('');
				//$('[name="level"]').val('');
				$('[name="password"]').val('');
